...
const TasksPage = () => {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-space-dark via-blue-900 to-purple-900 p-3 pb-24">
      <div className="max-w-md mx-auto space-y-4">
        ...
        <div className="space-y-3 my-0">
          {tasks.map(task => <Card key={task.id} className={`glass-card border-2 ${getTaskTypeColor(task.type)} ${task.completed ? 'opacity-60' : ''}`}>
              ...
              <CardContent className="pt-0 mx-0 py-[9px] my-[4px]">
                <p className="text-gray-300 mb-4 text-sm leading-relaxed">{task.description}</p>
                <Button onClick={() => handleCompleteTask(task.id)} disabled={task.completed} className={`w-full h-12 text-sm font-medium ${task.completed ? 'bg-green-600 hover:bg-green-600' : 'space-button'}`}>
                  {task.completed ? <>
